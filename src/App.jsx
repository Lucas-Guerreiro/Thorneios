...
import { 
  ...
  getAtletaById, getParticipacoesByAtleta, getParticipacoesByData, 
  calcularEstatisticasData, calcularClassificacaoData, resizeImage, 
  compressBase64, otimizarTodoEstado, getPlayerName, getDirectImageUrl, 
  getOneDriveEmbedUrl, isImageUrl, getEmbedUrl, handleOpenExternalLink, 
  escapeHtmlGlobal, downloadCsv, downloadXls, playWhistleSound 
} from "./utils/helpers";

import { 
  initStandings, recalcStandings, generateRR, phaseName, generateKO 
} from "./utils/classificacao";

import { 
  separarAtletasSorteio, agruparUnidades, drawBalancedTeams 
} from "./utils/sorteador";

import { 
  buildInitialPeladaState, obterCandidatosEmprestimoProximaPartida, 
  obterTimeDoador, startNextMatch, deduplicarEstadoPelada, 
  getVitoriasSeguidas, resolveMatch, higienizarJogadoresDuplicados, 
  higienizarFilaTimes, sincronizarBasesDosTimes 
} from "./utils/peladaState";
...
function MatchTimer({ t, defaultMinutes = 10, timerKey, onTimerUpdate }) {
  const {
    ...
    handleAddMinute,
    handleSubMinute,
  } = useMatchTimer(defaultMinutes, timerKey, onTimerUpdate);
